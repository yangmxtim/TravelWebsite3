<template>
  <div class="mb-3 items-center input-group">
    ...
    <input type="text" class="myInput" v-model="input1" style="width: 240px" 
      placeholder="搜尋" @keydown.enter="search" 
    />
  </div>
        ...
        <tr v-for="user in users" :key="user.id">
          ...
          <td>
            <button type="button" class="btn" data-bs-toggle="modal" 
              data-bs-target="#staticBackdrop" style="background-color: powderblue" 
  ...
  <div class="d-flex align-items-center justify-content-center pt-3">
    <button class="btn" style="background-color: khaki" 
...
const fetchMessage = async () => {
  ...
  try {
    const response = await axios.get(`http://localhost:8080/memberManage?q=${input1.value}`);
...
const search = () => {
  axios
    .get(`http://localhost:8080/memberManage?q=${input1.value}`)
    .then((response) => {
